...
void tui::report_interface(Database& db) {
	...
	auto daily_button = Button("DNEVNI IZVJESTAJ", [&] { daily_report(db); });
	auto weekly_button = Button("SEDMICNI IZVJESTAJ", [&] { weekly_report(db); });
	auto monthly_button = Button("MJESECNI IZVJESTAJ", [&] { monthly_report(db); });
	auto yearly_button = Button("GODISNJI IZVJESTAJ", [&] { yearly_report(db); });
	auto arbitrary_button = Button("PROIZVOLJNI IZVJESTAJ", [&] { arbitrary_report(db); });
...
void tui::arbitrary_report(Database& db) {

}

void tui::daily_report(Database& db) {
	std::string day;
	...
	auto generate_button = Button("GENERISI IZVJESTAJ", [&] { 
			...
			if (check) {
				if (util::is_valid_date(i_day, i_month, i_year)) {
					std::string temp =
	...
	auto renderer = Renderer(component, [&] {
		return vbox({vbox({
			vbox({center(hbox(text("DNEVNI IZVJESTAJ"))),
				  center(hbox(text("Izaberi datum") | underlined))}) | borderLight,
...
void tui::weekly_report(Database& db) { 
		std::string day;
	Component day_input = Input(&day, "     ");
	std::string month;
	Component month_input = Input(&month, "     ");
...
void tui::monthly_report(Database& db) {
	...
	auto component =
		Container::Vertical({cancel_button, generate_button, month_input, year_input});
...
void tui::yearly_report(Database& db) {
	std::string year;
	Component year_input = Input(&year, "     ");
